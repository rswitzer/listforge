...
Every production change follows Red → Green → Refactor: write the failing test first, make it pass with the smallest change, then refactor. The `.claude/hooks/check-tdd.sh` PreToolUse hook **blocks** writes to production files when no matching test file exists on disk, and the `tdd-reviewer` agent catches drift at PR time.
...
The pre-push hook at `.githooks/pre-push` must **stay enabled**: `.devcontainer/post-create.sh` keeps invoking `./scripts/setup-hooks.sh` (which sets `core.hooksPath=.githooks`), `.githooks/pre-push` is not deleted or stripped of its existing suites, and contributors don't unset `core.hooksPath` to bypass the gate. `LISTFORGE_SKIP_PREPUSH=1` stays a per-push emergency bypass, not a normalized workflow; `LISTFORGE_SKIP_PREPUSH_E2E=1` is for non-devcontainer environments only — inside the devcontainer Chromium libs are pre-installed, so E2E is expected to pass. Full contract: `docs/architecture.md §Development Environment`.
...
The devcontainer at `.devcontainer/` is the **required** dev environment — see `docs/architecture.md §Development Environment` for the full contract (canonicality, the same-change sync rule for new dev prerequisites, and the rules around keeping the pre-push hook enabled). From VS Code: command palette → **Dev Containers: Reopen in Container**. `postCreateCommand` runs `.devcontainer/post-create.sh` (restores .NET, runs `pnpm install`, apt-installs Chromium runtime libs + Xvfb, runs `playwright install chromium`, and runs `scripts/setup-hooks.sh` to enable the pre-push gate). When you pull a branch that changes `package.json`/`pnpm-lock.yaml`, accept the rebuild prompt — `updateContentCommand` re-runs `pnpm install` and `playwright install chromium` automatically. Inside the devcontainer there is no need to run `npx playwright install` or set `LISTFORGE_SKIP_PREPUSH_E2E`.
